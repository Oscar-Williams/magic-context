...
import { existsSync, mkdirSync, readFileSync, readdirSync, renameSync, writeFileSync, mkdtempSync, rmSync } from "node:fs";
import { tmpdir } from "node:os";
import { join } from "node:path";

const FILES = ["ARCHITECTURE.md", "STRUCTURE.md"] as const;
const hash = (text: string) => createHash("sha256").update(text).digest("hex");
const proposalDir = (projectDir: string) => join(projectDir, ".cortexkit/magic-context/docs-update-proposals");

export function docsBaseHashes(projectDir: string): Record<string, string> {
    return Object.fromEntries(FILES.map((name) => [name, hash(existsSync(join(projectDir, name)) ? readFileSync(join(projectDir, name), "utf8") : "")]));
}

export function listPendingDocsProposals(projectDir: string): string[] {
    const dir = proposalDir(projectDir);
    return existsSync(dir) ? readdirSync(dir).filter((name) => name.endsWith(".md")).sort().map((name) => join(dir, name)) : [];
}
...
export function hasCurrentDocsProposal(projectDir: string): boolean {
    ...
    return listPendingDocsProposals(projectDir).some((path) =>
        Object.entries(hashes).every(([name, value]) => readFileSync(path, "utf8").includes(`Base SHA-256 ${name}: ${value}`)),
    );
}

function git(projectDir: string, args: string[]): string {
    return execFileSync("git", args, { cwd: projectDir, encoding: "utf8", timeout: 10000, maxBuffer: 2_000_000 }).trim();
}

export function docsChangeSet(projectDir: string, storedAnchor?: string): { head: string; text: string } | null {
    try {
        if (git(projectDir, ["rev-parse", "--show-toplevel"]) !== projectDir) return null;
        const head = git(projectDir, ["rev-parse", "HEAD"]);
        let anchor = storedAnchor;
        if (!anchor || !/^[a-f0-9]{40}$/.test(anchor) || !git(projectDir, ["cat-file", "-t", anchor]).includes("commit")) anchor = git(projectDir, ["log", "-1", "--format=%H", "--", ...FILES]);
        if (!anchor) return null;
        const excluded = ["ARCHITECTURE.md", "STRUCTURE.md", ".cortexkit", "*.lock", "*lock.json", "*test*", "*spec*", "dist", "*generated*"];
        const raw = git(projectDir, ["log", "--format=commit %h %s", "--stat", "--no-renames", "-n", "201", `${anchor}..HEAD`, "--", ".", ...excluded.map((path) => `:(exclude)${path}`)]);
        if (!raw) return null;
        const commits = raw.split(/^commit /m);
        const text = raw.slice(0, 24000);
        return { head, text: `${text}${raw.length > text.length ? `\n[cut ${raw.length - text.length} bytes]` : ""}${commits.length > 201 ? "\n[cut commits beyond newest 200]" : ""}` };
    } catch { return null; }
}
...
function protectedBytes(text: string): string[] {
    return [...text.matchAll(/<!-- mc:protected START[^\n]*-->[\s\S]*?<!-- mc:protected END -->/g)].map((match) => match[0]);
}
...
function diff(name: string, before: string, after: string): string {
    ...
    try {
        ...
        try {
            return execFileSync("git", ["diff", "--no-index", "--", join(dir, "before"), join(dir, "after")], { encoding: "utf8" });
        } catch (error) {
            return (error as { stdout?: string }).stdout?.replaceAll(join(dir, "before"), `a/${name}`).replaceAll(join(dir, "after"), `b/${name}`) ?? "";
        }
    } finally { rmSync(dir, { recursive: true, force: true }); }
}

export function validateDocsProposal(projectDir: string, output: string, budget: number, expected: Record<string, string>): { changes: DocsSectionChange[]; tokens: number; diffs: string } {
    if (Object.entries(expected).some(([name, value]) => docsBaseHashes(projectDir)[name] !== value)) throw new Error("base drifted");
    ...
    for (const file of FILES) {
        const before = existsSync(join(projectDir, file)) ? readFileSync(join(projectDir, file), "utf8") : "";
        let after = before;
        const seen = new Set<string>();
        for (const change of changes.filter((item) => item.file === file)) {
            if (!(["replace", "add", "remove"] as unknown[]).includes(change.action) || typeof change.heading !== "string" || !/^#{1,6} \S/.test(change.heading) || typeof change.text !== "string" || typeof change.reason !== "string" || !change.reason.trim() || seen.has(change.heading)) throw new Error(`invalid section change in ${file}`);
            ...
            if (change.action === "add") {
                ...
                if (change.action === "replace" && (!change.text.startsWith(change.heading + "\n") || sections(change.text).length !== 1)) throw new Error(`invalid replacement ${change.heading}`);
                const lines = after.split("\n");
                lines.splice(section.start, section.end - section.start, ...(change.action === "remove" ? [] : change.text.trimEnd().split("\n")));
                after = lines.join("\n");
            }
        }
        if (JSON.stringify(protectedBytes(before)) !== JSON.stringify(protectedBytes(after))) throw new Error(`protected region changed in ${file}`);
        ...
        if (JSON.stringify(sections(after).map((item) => item.heading)) !== JSON.stringify(expectedHeadings)) throw new Error(`markdown structure changed in ${file}`);
...
export function writeDocsProposal(projectDir: string, output: string, budget: number, expected: Record<string, string>, head: string): string {
    ...
    for (const old of listPendingDocsProposals(projectDir)) if (old !== path) renameSync(old, join(dir, "superseded", old.split("/").at(-1)!));
    return path;
}
